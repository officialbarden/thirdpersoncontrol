...
# FORWARDS:
    ...
    $execute as @s[predicate=tpc:input/sprint] store result score @s tpc.MovementSpeed run data get storage tpc:entities entity_type[{"name":"$(name)"}].movement.run 1000
    execute as @s[predicate=tpc:bool/wasd_check] run return run function tpc:private/control/move/vector
